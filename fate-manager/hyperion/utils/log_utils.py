...
from utils.conf_utils import get_project_base_directory
...
class LoggerFactory(object):
    ...
    NO_FORMAT = "[%(levelname)s] [%(asctime)s] : %(message)s"
    LEVEL = logging.DEBUG
    logger_dict = {}
    global_handler_dict = {}
    ...
    def set_directory(directory=None, parent_log_dir=None, append_to_parent_log=None, force=False):
        ...
        with LoggerFactory.lock:
            if not directory:
                ...
                directory = os.path.abspath(os.path.join(get_project_base_directory(), 'logs'))
    ...
    @staticmethod
    def get_handler(class_name, level=None, log_dir=None, log_type=None, job_id=None, formatter_disable=False):
        if not log_type:
    ...
    def get_schedule_logger(job_id='', log_type='schedule', formatter_disable=False):
        ...
        for job_log_dir in log_dirs:
            handler = LoggerFactory.get_handler(class_name=None, level=LoggerFactory.LEVEL,
                                                ...
                                                formatter_disable=formatter_disable)
    ...
    def get_host_logger(job_id, host):
        ...
        handler = LoggerFactory.get_handler(class_name=host, level=LoggerFactory.LEVEL,
                                            log_dir=job_log_dir, job_id=job_id)
        error_handler = LoggerFactory.get_handler(class_name=host, level=logging.ERROR,
                                                  log_dir=job_log_dir, job_id=job_id)
        logger.addHandler(handler)
        logger.addHandler(error_handler)
        with LoggerFactory.lock:
            LoggerFactory.host_logger_dict[job_id + host] = logger
        return logger
