...
from fate_manager.utils import file_utils
...
class LoggerFactory(object):
    ...
    def get_handler(class_name, level=None, log_dir=None, log_type=None, job_id=None):
        if not log_type:
            ...
            log_file = os.path.join(log_dir, "fate_manager_{}.log".format(
                log_type) if level == LoggerFactory.LEVEL else 'fate_manager_{}_error.log'.format(log_type))
        if job_id:
    ...
    def get_schedule_logger(job_id='', log_type='schedule'):
        fate_manager_log_dir = os.path.join(file_utils.get_project_base_directory(), 'logs', 'fate_manager')
        job_log_dir = os.path.join(file_utils.get_project_base_directory(), 'logs', job_id)
        if not job_id:
            log_dirs = [fate_manager_log_dir]
        else:
            if log_type == 'audit':
                log_dirs = [job_log_dir, fate_manager_log_dir]
            else:
                log_dirs = [job_log_dir]
        os.makedirs(job_log_dir, exist_ok=True)
        os.makedirs(fate_manager_log_dir, exist_ok=True)
...
def schedule_logger(job_id=None, delete=False):
    if not job_id:
        return getLogger("fate_manager_schedule")
    else:
        if delete:
            with LoggerFactory.lock:
                try:
