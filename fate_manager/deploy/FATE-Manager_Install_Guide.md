...
**注意：下文介绍的MiniKube机器IP地址是`xxx.xxx.xxx.xxx`。请修改为你准备的实验机器IP地址！！！**
...
ingress.extensions/kubefate   <none>   kubefate88.net   xxx.xxx.xxx.xxx   80      19d
```
...
注意：下面地址xxx.xxx.xxx.xxx 要替换为你的MiniKube机器地址。
```
sudo -- sh -c "echo \"xxx.xxx.xxx.xxx kubefate.net\"  >> /etc/hosts"
```
添加完毕后，可以验证是否生效：
```
[app@kube-m1 kubefate]$ ping -c 2 kubefate.net
PING kubefate88.net (xxx.xxx.xxx.xxx) 56(84) bytes of data.
64 bytes from kube-n1 (xxx.xxx.xxx.xxx): icmp_seq=1 ttl=64 time=0.729 ms
64 bytes from kube-n1 (xxx.xxx.xxx.xxx): icmp_seq=2 ttl=64 time=0.710 ms
...
ExchangeIp=xxx.xxx.xxx.xxx                    #路由服务出入口ip
