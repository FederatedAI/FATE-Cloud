    ...
    <div v-if="activeName==='Auto test'" class="deploying-body">
        ...
        <div class="table">
            <el-table
                ...
                >
                <el-table-column prop="index" class-name="index" :label="$t('Index')" width="100"></el-table-column>
                <el-table-column prop="testItem" :label="$t('Test item')" ></el-table-column>
                <el-table-column prop="duration" :label="$t('Test duration')" >
                     <template slot-scope="scope">
                            <span>{{scope.row.duration | durationFormat}}</span>
                        </template>
                </el-table-column>
                <el-table-column prop="status.desc" align="center"  :label="$t('Status')" >
                    <template slot-scope="scope">
...
import { ansibleClick, ansibleTestList, ansibleAutoTest } from '@/api/fatedeployAnsible'
...
export default {
    ...
    methods: {
        async initiTextList() {
            ...
            let res = await ansibleTestList(data)
            // 判断返回值是否为空
        ...
        toStart() {
            ...
            ansibleAutoTest(dataParams).then(res => {
                this.lessTime()
            })
        },
        // //并行测试，0.3版改为串行
